...
internal static class Registration
{
    ...
    private static string RegisterTypes(ImmutableList<DataPackage> datas, string targetTypeShort, string[] classData)
    {
        ...
        foreach (var data in datas)
        {
            if (CreateFromParent.CreateIndexArray(classData,data.TypeParameters) != null)
            {
                ...
                sb.AppendLine($"NewSerializerRegistry.Instance.Register(this,typeof({targetTypeShort}),typeof({data.ShortDisplayString}));");
    ...
    public static string CreateRegisterThis(this ClassPackage package)
    {
        ...
        sb.AppendLine($"NewSerializerRegistry.Instance.RegisterTag($\"{package.ClassInfo.NameSpace}.{package.ClassInfo.TypeName},{package.ClassInfo.NameSpace.Split('.')[0]}\",typeof({package.ClassInfo.ShortDefinition}));");
        ...
        sb.AppendLine($"NewSerializerRegistry.Instance.Register(this,typeof({package.ClassInfo.ShortDefinition}),typeof({package.ClassInfo.ShortDefinition}));");
        ...
        if (package.ClassInfo.AliasTag != "")
        {
            ...
            sb.AppendLine($"NewSerializerRegistry.Instance.RegisterTag(\"{package.ClassInfo.AliasTag}\",typeof({package.ClassInfo.ShortDefinition}));");
        ...
        if (package.ClassInfo.IsGeneric)
        {
            ...
            sb.AppendLine($"NewSerializerRegistry.Instance.RegisterGenericFormatter(typeof({package.ClassInfo.ShortDefinition}),typeof({package.ClassInfo.GeneratorName + package.ClassInfo.TypeParameterArgumentsShort}));");
            sb.AppendLine($"{Constants.SerializerRegistry}.RegisterFormatter(typeof({package.ClassInfo.ShortDefinition}));");
            sb.AppendLine($"NewSerializerRegistry.Instance.RegisterFormatter(typeof({package.ClassInfo.ShortDefinition}));");
            return sb.ToString();
        }
        sb.Append("\t\tvar formatter = new ").Append(package.ClassInfo.NameSpace).Append('.').Append(package.ClassInfo.GeneratorName).AppendLine("();");
        sb.Append("NewSerializerRegistry.Instance").AppendLine(string.Format(Constants.RegisterFormatter, "formatter")).ToString();
